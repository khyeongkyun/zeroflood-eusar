...
# ======== Slurm setting
#SBATCH -J pga_ft_tr_b_zf_tim_chain_dls
#SBATCH --output=pga_ft_tr_b_zf_tim_chain_dls_%j.out
#SBATCH --error=pga_ft_tr_b_zf_tim_chain_dls_%j.err
...
srun torchrun --nnodes=1 --nproc_per_node=1 --master_port $MASTER_PORT \
   ...
   'encoder.tim_modalities=[DEM,LULC,S2L2A]' \
   decoder=seg_upernet \
