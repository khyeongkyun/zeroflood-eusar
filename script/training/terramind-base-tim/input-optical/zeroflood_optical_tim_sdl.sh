...
# ======== Slurm setting
#SBATCH -J pga_ft_tr_b_zf_tim_chain_sdl
#SBATCH --output=pga_ft_tr_b_zf_tim_chain_sdl_%j.out
#SBATCH --error=pga_ft_tr_b_zf_tim_chain_sdl_%j.err
...
srun torchrun --nnodes=1 --nproc_per_node=1 --master_port $MASTER_PORT \
   ...
   'encoder.tim_modalities=[S1RTC,DEM,LULC]' \
   decoder=seg_upernet \
