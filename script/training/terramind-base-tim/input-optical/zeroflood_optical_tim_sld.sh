...
# ======== Slurm setting
#SBATCH -J pga_ft_tr_b_zf_tim_chain_sld
#SBATCH --output=pga_ft_tr_b_zf_tim_chain_sld_%j.out
#SBATCH --error=pga_ft_tr_b_zf_tim_chain_sld_%j.err
...
srun torchrun --nnodes=1 --nproc_per_node=1 --master_port $MASTER_PORT \
   ...
   'encoder.tim_modalities=[S1RTC,LULC,DEM]' \
   decoder=seg_upernet \
