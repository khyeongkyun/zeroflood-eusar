...
def process_v2(
    ...
    dataset = build_terramesh_dataset(
        path=f"{root}/TerraMesh",  # Streaming or local path
        modalities=['S1GRD', "S2L2A", 'S2RGB', "LULC", "DEM"],  
        split=terramesh_split,
        shuffle=False,  # Set false for split="val"
        batch_size=1,
        return_metadata=True,
    ...
    for idx, batch in enumerate(dataloader):
        try:
            ...
            if idx % 1000 == 0:
                ...
                # 0             0               0               0
            keys = list(batch.keys())
            if not ('time_S1GRD' in keys and 'time_S2RGB' in keys):
                continue
            if max_samples != -1 and sample_count >= max_samples:
                break
            ...
            time_S1GRD, time_S2L2A = int(batch['time_S1GRD'][0]), int(batch['time_S2L2A'][0])
            lon, lat = batch['center_lon'][0].item(), batch['center_lat'][0].item()
            x, y = batch['x'][0].tolist(), batch['y'][0].tolist()
            crs = batch['crs'][0].item()
            
            S1GRD, S2L2A = batch['S1GRD'].squeeze(), batch['S2L2A'].squeeze()
            RGB, LULC = batch['S2RGB'].squeeze().byte(), batch['LULC'].squeeze().reshape(1, 264, 264)
            DEM = batch['DEM'].squeeze().reshape(1, 264, 264) 
            ...
            # Save processed data
            if save_img:
                out_dir = Path(f"{root}/ZeroFlood_v2/{terramesh_split}")
                out_dir.mkdir(parents=True, exist_ok=True)
                info = [
                    ("S1GRD", S1GRD, time_S1GRD),
                    ("S2L2A", S2L2A, time_S2L2A),
                    ("S2RGB", RGB, time_S2L2A),
                    ("LULC", LULC, time_S2L2A),
                    ("DEM", DEM, time_S2L2A),
            ...
            if save_metadata:
                writer.writerow(
                    [
                        keys,
                        timestamp_to_str(time_S1GRD), timestamp_to_str(time_S2L2A),
                        (int(TN), int(FN), int(FP), int(TP)),
                        (lon, lat), (x[0], y[0]), crs
                    ]
                )
            del S1GRD, S2L2A, RGB, LULC, LULC_W, DEM, flood_mask, water_mask, spurious_mask
